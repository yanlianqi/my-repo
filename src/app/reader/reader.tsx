        ...
        <article className={styles.reader} ref={articleRef}>
          ...
          <div className={styles.readingColumn} key={current?.id}>
            ...
            <div className={styles.pageNav}>
              <button disabled={activeIndex === 0} onClick={() => selectChapter(chapters[activeIndex - 1])}>← <span>上一集</span></button>
              <span>{activeIndex + 1} / {chapters.length}</span>
              <button disabled={activeIndex === chapters.length - 1} onClick={() => selectChapter(chapters[activeIndex + 1])}><span>下一集</span> →</button>
            </div>
          </div>
        </article>
